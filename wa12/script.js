...
function displayEvents(events, city) {
  ...
  events.forEach(event => {
    ...
    div.innerHTML = `
      <h2>${event.name}</h2>
      <p>${event.dates?.start?.localDate ? new Date(event.dates.start.localDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "No date"}</p>
      ${img ? `<img src="${img}" alt="Image of Performing Act" />` : ""}
      <p><a href="${event.url}" target="_blank" class="ticket-link">Purchase Tickets on Ticketmaster</a></p>`;
